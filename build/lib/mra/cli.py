...
科研中间体 · 使用流程
  ...
  先试手感            mra demo                        # 复制示例文献+数据到当前目录

  不需要 API key 的命令：demo / lint / refs / memory / usage / status / guide
                        （import 处理 .xml 时也不需要）
"""
...
EXAMPLE_FILES = ("demo_corpus.xml", "demo_data.csv", "demo_notes.md")


def cmd_demo(args, cfg: Config) -> int:
    """Copy the bundled sample files here, so trying the tool needs no path hunt.

    They ship inside the package rather than only in the repository, because
    telling someone who pip-installed to run `mra import examples/...` sends
    them looking for a directory they do not have.
    """
    import shutil
    ...
    print(f"复制了 {len(copied)} 个示例文件到 {target.resolve()}")
    for path in copied:
        print(f"  {path.name}")
    print(
        "\n试试看（前两条不花钱）：\n"
        f"  mra import {EXAMPLE_FILES[0]}\n"
        "  mra status\n"
        f"  mra figures {EXAMPLE_FILES[1]} {EXAMPLE_FILES[2]}\n"
        f"  mra assess {EXAMPLE_FILES[1]} {EXAMPLE_FILES[2]}"
    )
    return 0
