...
它会自己检查 Python、装好依赖、问你用哪家模型并要一次 key，然后给一个中文菜单。
**第一项就是网页界面** —— 选 1，浏览器自动打开，剩下的都在页面里点：

```
 1  网页界面      推荐 —— 在浏览器里操作，不用记命令

 2  导入并阅读    PDF / PubMed XML / 纯文本 —— 读完直接出分析
 3  提炼文献      逐篇结构化提炼
 4  科学对话      基于你的文献库追问
 5  评估数据      打分 + 推荐候选期刊
 6  文献列表      看库里有什么、每篇提炼出了什么（不花钱）
 7  引用核对      检查文稿引用是否真实（不花钱）
 8  试用示例      导入自带的 8 篇示例文献（不花钱）
 9  连接自检      模型连不通时先跑这个
10  打开数据目录
11  查看状态      文献数、假说、花费
12  更换 API key  换服务商，或换掉已经作废的 key
13  所有课题      横看每个课题走到哪一步（不花钱）
14  切换 / 新建课题
```

模型那一步给两个选项：**Claude**（判断质量最好）或 **DeepSeek**（便宜很多，全部命令
都实测跑通过）。选 DeepSeek 的话，多出来的那个依赖和四个环境变量都由它装好配好——
不用照着 [docs/PROVIDERS.md](docs/PROVIDERS.md) 手配。想换回 Claude，把 `MRA_PROVIDER`
这个环境变量删掉即可。
...
```bash
mra web            # 浏览器自动打开，地址是 127.0.0.1
```

一个只监听**本机回环地址**的小服务。文献库、数据、稿件**全部还在你自己的硬盘上**，
局域网里的别人访问不到，更不用说外网 —— 和命令行版本是同一条数据边界（QA-3）。

页面把命令分成四组：文献库 / 想清楚 / 写出来 / 工具。会花钱的那些在侧栏标了 `$`。
文件不用手打路径，点「选择文件…」在页面里挑；对话是聊天窗口的样子；每一步的输出实时
往下滚，和终端里看到的一模一样 —— 因为**它跑的就是同一条命令**，网页只是壳。

> 启动时终端里会打印一个带 `?t=…` 的网址，**要用那个完整网址打开**。那个 token 是防
> 止你浏览器里别的标签页在后台驱动这个工具（它能花钱、能读你的文件）。跨站请求一律拒绝。
...
一个 PI 同时带几个课题，它们的文献库不该混在一起——关于肝纤维化的假说，不应该在问胰腺癌
时被检索出来。所以**一个课题一个文件夹，各自一个知识库**：

```
workspace/                    ← 启动器旁边
├── .mra/                     ← 「默认」课题（老的单课题布局，照常能用）
├── 肝纤维化/
│   └── .mra/
└── 胰腺癌-张三/
    └── .mra/
...
```bash
mra project new "肝纤维化"                    # 新建
mra --project 肝纤维化 import paper.pdf --digest
mra project list                              # 横着看一眼
```

网页界面**右上角有下拉框**切换，旁边「＋ 新建」建新的。启动器里是第 13、14 项。

`project list` 回答的是 PI 真正会问的那几个问题——谁在动、读了多少、走到哪一版假说、
各花了多少钱，并且**点名哪些课题导了文献却一篇都没读**：

```
课题                  文献   已读   假说   花费      当前假说
────────────────────────────────────────────────────────────
▸ 默认                8      8      2      $1.24     髓系 TGF-β1 驱动门脉纤维化
  肝纤维化            41     41     1      $3.02     TREM2+ 巨噬细胞限制而非促进纤维化
  胰腺癌-张三         12     0      0      $0.31     —

这几个导了文献但一篇都没读：胰腺癌-张三
```

**没有注册表文件**——课题就是硬盘上的文件夹，扫出来的。注册表在有人重命名或拷贝文件夹
的那一刻就会和真实情况脱节，然后工具会对着不存在的东西信誓旦旦。
...
**长对话不会静默失忆。** 磨假说本来就是二十轮起步的事，而模型的上下文放不下全部。
之前的做法是**硬截断**——最早那些轮直接消失，不留痕迹，于是第 21 轮会重新提出第 3 轮
已经排除掉的方向，而你不知道为什么。

现在超出窗口的部分会**折进一份摘要**，而不是丢掉。摘要专门保留最贵的那几样：

- **排除过什么，以及排除的理由**（这是整场对话里最难重建的东西——没有它，
  同一个机制会被反复提出来）
- 哪些文献被当成决定性的，支持的和反证的分开列，`[PMID:x]` 原样保留
- 提出过的证伪方案
- 你说过的、关于自己未发表数据的事实（队列大小、做过什么没做过什么）——
  这些在文献里查不到，丢了就没了

折叠**攒够十条才做一次**（不是每轮都做），失败了就退回原状并告警，不会毁掉这一轮。
折了多少条会打印出来，`mra status` 也会显示。
...
**检索在字面对不上的时候会自己拓宽。** BM25 是字面匹配，所以两种情况会漏：中文提问在
英文语料上得分为零；英文提问用的词和论文不一样——你问 senescence，论文写 SASP；
你问"成瘢痕的细胞"，论文写 myofibroblast。

触发条件不是语言，是**没检索满**：literal 查询返回的篇数少于库里能给的，才花约 $0.01
买一组替代检索词（符号、缩写全称、术语、期刊惯用拼法），再检一次。**已经检满就不花这笔钱**，
库是空的也不花。

合并方式是**字面匹配在前，拓宽找到的接在后面**。不是替换、也不混着排——你手打的 `TREM2`
几乎总比任何转述更准，让同义词把它挤下去是拿精确度换召回率，方向错了。

拓宽发生时上下文头部会写明"literal 没匹配满，另外还搜了：…"，这样模型知道后面那几篇
可能没那么切题，而不是把它们当成同等命中。调用失败也毁不掉这一轮——退回到"最近导入的几篇"，
并注明相关性未经证实。
...
之前是**测不出来的**，只能人肉并排读两份输出。

材料让这件事很便宜，因为**缺陷是故意埋的**：`demo_notes.md` 里写了一个 `demo_data.csv`
根本没有的数字；示例语料里有互相矛盾的证据；`evals/cross_sectional.txt` 是一篇我们自己
编的横断面研究，特意写成因果表述过头、不报效应量、机制全靠计算。答案已知，所以是
**通过/不通过**，而不是拿一个模型去给另一个模型打分。

```
✗ digest-cross-sectional  4/6
      ✓ 指出横断面设计撑不起因果表述
      ✗ 提出反向因果的可能            ← 变差了
      ✓ 指出摘要里没有效应量
      ...
没抓到的：
  · [digest-cross-sectional] 提出反向因果或共同处置的可能
```

仓库里带了一份 **claude-opus-5 的参考基线**（2026-08，18/18，$0.72），所以换模型时
不必先自己跑一遍：

```bash
MRA_PROVIDER=openai mra eval --baseline mra/evals/baseline-claude-opus-5.json
```

**满分是个信号，不是成绩。** 18/18 说明这套用例对 Claude 已经**饱和**——它现在只能
发现退步，发现不了进步。而如果 DeepSeek 也跑到 18/18，**不能得出"两者一样好"**，
只能得出"这套用例太容易，分不出来"，那时候要做的是加更难的用例。

**它量的是什么，说清楚：** 一条通过 = 输出里出现了对应说法。这是"做到了"的**必要条件，
不是充分条件**——模型可以写出"反向因果"四个字却没真论证，这里也会算它通过。但反过来
是可靠的：**没提到就是确实没说**。所以高分不能当成好，掉分是真的掉了。这个不对称正是
它能抓回归的原因。

`--free-only` 那两条不花钱也不用 key，任何人任何时候都能跑。
...
```bash
mra import ~/papers/tgfb1.pdf --digest
```

`--digest` 让它导入之后接着读，并把结果直接打出来——科学问题 / 关键发现 / 方法 /
作者主张的新意 / **局限（含作者没承认的）** / 临床相关性 / 证据强度。
网页界面里这是「导入并阅读」，那个勾**默认是勾上的**。

不加 `--digest` 就只入库不读，屏幕上只报"入库几篇"。批量导几百篇时这是对的——
先都收进来，之后再挑着读。

读过的东西随时用 `mra library` 回看：

```bash
mra library              # 列出全部，「提炼」那一列 ✓ 表示已经读过
mra library 31234567     # 看某一篇的完整提炼结果
mra library local:a1b2c3d4
```

网页界面里这是「文献列表」，列表里的编号可以直接点开。
